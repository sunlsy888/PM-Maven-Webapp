package com.lsy.ritu.controller.system.role;
...
public class RoleController extends BaseController {
	...
	public String auth(@RequestParam String ROLE_ID,Model model)throws Exception{
		...
		try{
			...
			logBefore(logger, "auth json:"+json.toString());
	...
	public ModelAndView button(@RequestParam String ROLE_ID,@RequestParam String msg,Model model)throws Exception{
		...
		try{
			...
			else if("up_qx".equals(msg)){
				roleRights = role.getUP_QX();
			}
			else if("down_qx".equals(msg)){
				roleRights = role.getDOWN_QX();
			}
			else if("verify_qx".equals(msg)){
				roleRights = role.getVERIFY_QX();
			}else if("reply_qx".equals(msg)){
				roleRights = role.getREPLY_QX();
			}else if("confirm_qx".equals(msg)){
				roleRights = role.getCONFIRM_QX();
			}
		
			logBefore(logger, "button menuList:"+menuList.toString());
			logBefore(logger, "button roleRights:"+roleRights.toString());
			if(Tools.notEmpty(roleRights)){
				for(Menu menu : menuList){
					menu.setHasMenu(RightsHelper.testRights(roleRights, menu.getMENU_ID()));
					if(menu.isHasMenu()){
			...
			JSONArray arr = JSONArray.fromObject(menuList);
			String json = arr.toString();
			logBefore(logger, "button json:"+json.toString());
		...
		try{
			if(Jurisdiction.buttonJurisdiction(menuUrl, "edit")){
				if(null != menuIds && !"".equals(menuIds.trim())){
					...
					logBefore(logger, "auth :"+pd.toString());
	...
	public void orleButton(@RequestParam String ROLE_ID,@RequestParam String menuIds,@RequestParam String msg,PrintWriter out)throws Exception{
		...
		try{
			if(Jurisdiction.buttonJurisdiction(menuUrl, "edit")){
				logBefore(logger, "roleButton menuUrl:"+menuUrl.toString());
				
				logBefore(logger, "roleButton menuIds:"+menuIds.toString());
				if(null != menuIds && !"".equals(menuIds.trim())){
					BigInteger rights = RightsHelper.sumRights(Tools.str2StrArray(menuIds));
				...
				logBefore(logger, "roleButton :"+pd.toString());
