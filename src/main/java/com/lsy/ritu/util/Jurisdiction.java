			...
			for(int j=0;j<menuList.get(i).getSubMenu().size();j++){
				if(menuList.get(i).getSubMenu().get(j).getMENU_URL().split(".do")[0].equals(menuUrl.split(".do")[0])){
					if(!menuList.get(i).getSubMenu().get(j).isHasMenu()){				//判断有无此菜单权限
						...
						map.put("up", RightsHelper.testRights(map.get("ups"), MENU_ID) || isAdmin?"1":"0");
						map.put("down", RightsHelper.testRights(map.get("downs"), MENU_ID) || isAdmin?"1":"0");
						map.put("verify", RightsHelper.testRights(map.get("verifys"), MENU_ID) || isAdmin?"1":"0");
						map.put("reply", RightsHelper.testRights(map.get("replys"), MENU_ID) || isAdmin?"1":"0");
						map.put("confirm", RightsHelper.testRights(map.get("confirms"), MENU_ID) || isAdmin?"1":"0");
				...
				if(menuList.get(i).getSubMenu().get(j).getMENU_URL().split(".do")[0].equals(menuUrl.split(".do")[0])){
					if(!menuList.get(i).getSubMenu().get(j).isHasMenu()){				//判断有无此菜单权限
						...
						if("add".equals(type)){
							...
							return ((RightsHelper.testRights(map.get("ups"), MENU_ID)) || isAdmin);
						}else if("down".equals(type)){
							return ((RightsHelper.testRights(map.get("downs"), MENU_ID)) || isAdmin);
						}else if("verify".equals(type)){
							return ((RightsHelper.testRights(map.get("verifys"), MENU_ID)) || isAdmin);
						}else if("reply".equals(type)){
							return ((RightsHelper.testRights(map.get("replys"), MENU_ID)) || isAdmin);
						}else if("confirm".equals(type)){
							return ((RightsHelper.testRights(map.get("confirms"), MENU_ID)) || isAdmin);
						}
