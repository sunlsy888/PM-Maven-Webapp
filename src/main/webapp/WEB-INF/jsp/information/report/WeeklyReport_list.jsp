...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="report/listall.do" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								...
								<td class='center' style="width: 30px;">${vs.index+1}</td>
										...
										<td> <c:forEach items="${PTypeList}" var="dictionaries">
										 
							 <c:if test="${var.PType==dictionaries.ZD_ID}">${dictionaries.NAME }</c:if>
						</c:forEach> </td>
										<td>${var.PM}</td>
										<td>${var.PlanDate}</td>
										<td>${var.Requirement}</td>
										<td>${var.ItemDetails}</td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function del(Id){
			...
			if(confirm("确定要删除?")){ 
				top.jzts();
				var url = "<%=basePath%>report/delete.do?Id="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>report/goEdit.do?Id='+Id;
			 diag.Width = 800;
			 diag.Height = 565;
			 diag.CancelEvent = function(){ //关闭事件
			...
			if(confirm(msg)){ 
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>report/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>report/excel.do';
		}
		</script>
		<style type="text/css">
