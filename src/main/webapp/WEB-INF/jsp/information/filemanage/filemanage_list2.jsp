...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="filemanage/list.do" method="post" name="Form" id="Form">
			<table >
				<tr>
					<td>
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								...
								<td class='center' style="width: 30px;">${vs.index+1}</td>
										...
										<td><c:forEach items="${onlinedocumentationList}" var="dictionaries">
							 <c:if test="${var.FType==dictionaries.ZD_ID}">${dictionaries.NAME }</c:if>
						</c:forEach>  </td> 
										<td>${var.AddDate}</td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>filemanage/goAdd.do';
			 diag.Width = 800;
			 diag.Height = 490;
		...
		function del(Id,PATH){
			...
			if(confirm("确定要删除?")){ 
				top.jzts();
				var url = "<%=basePath%>filemanage/delete.do?Id="+Id+"&FilePath="+PATH+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>filemanage/goEdit.do?Id='+Id;
			 diag.Width = 600;
			 diag.Height = 465;
			 diag.CancelEvent = function(){ //关闭事件
			...
			if(confirm(msg)){ 
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>filemanage/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>filemanage/excel.do';
		}
