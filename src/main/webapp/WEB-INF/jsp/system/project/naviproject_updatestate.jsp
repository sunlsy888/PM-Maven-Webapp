...
<script type="text/javascript">
	...
	function save(){
		...
		if($("#PState").val()=="" ){
			...
			$("#PState").focus();
			$("#PState").val('');
			$("#PState").css("background-color","white");
			return false;
		}else{
			$("#PState").val(jQuery.trim($('#PState').val()));
		}
...
<body>
	<form action="project/updateState.do" name="projectForm" id="projectForm" method="post">
		<input name="Id" id="Id"  type="hidden"  value="${pd.Id}"/>
		<input type="hidden" name="PNo" id="PNo" value="${pd.PNo }"   title="项目编号"/>
		<div id="zhongxin">
		<table>
				
			<tr>
			 	 <td>
                   <select class="chzn-select" name="PState" id="PState" data-placeholder="请选择项目状态" style="vertical-align:top;"  >
						<option value=""></option>  
						<c:forEach items="${PSTATEList}" var="dictionaries">
							<option value="${dictionaries.NAME }" <c:if test="${pd.PState==dictionaries.NAME}">selected</c:if>>${dictionaries.NAME }</option>
						</c:forEach>  
					  	</select>	
</td> 
			</tr>
