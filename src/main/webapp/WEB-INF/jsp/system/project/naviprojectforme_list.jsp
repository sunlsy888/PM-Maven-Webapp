...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="project/cylist.do?PNo=${pd.PNo }" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								...
								<td class='center' style="width: 30px;">${vs.index+1}</td> 
										...
										<td>
   <c:if test="${QX.cha == 1 }">
											 <a style="cursor:pointer;" title="查看" href="project/pminfo.do?ID=${var.Id}"  target="_blank" class="tooltip-success" > ${var.PName}</a>
										</c:if>
						           <c:if test="${QX.cha == 0 }">
							          ${var.PName}
						                 </c:if> </td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function updatestate(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="更新";
			 diag.URL = "<%=basePath%>project/goUpdateState.do?ID="+Id;
			 diag.Width = 260;
			 diag.Height = 220;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>project/excel.do?type=me';
		}
		</script>
		...
		//导出excel
		function toExcelfor(){
			var PNAME = $("#nav-search-input").val(); 
			window.location.href='<%=basePath%>project/excel.do?PNAME='+PNAME;
		}
