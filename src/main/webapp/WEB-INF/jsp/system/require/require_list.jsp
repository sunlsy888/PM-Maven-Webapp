...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="require/list.do?PId=${pd.PId}" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								<td class='center' style="width: 30px;">
									<label><input type='checkbox' name='ids' value="${var.Require_Id}" /><span class="lbl"></span></label>
								</td>
								<td class='center' style="width: 30px;">${vs.index+1}</td> 
										  <td> <c:if test="${QX.cha == 1 }">
											 <a style="cursor:pointer;" title="查看" href="require/reinfo.do?Require_Id=${var.Require_Id}"  target="_blank" class="tooltip-success" > ${var.ItemTitle}</a>
										</c:if>
						           <c:if test="${QX.cha == 0 }">
							          ${var.ItemTitle}
						                 </c:if>  </td>
								...
								<td style="width: 30px;" class="center">
										...
										<div>
											<c:if test="${QX.edit == 1 }">
											<li><a style="cursor:pointer;" title="编辑" onclick="edit('${var.Require_Id}');" class="tooltip-success" data-rel="tooltip" title="" data-placement="left"><span class="green"><i class="icon-edit"></i></span></a></li>
											</c:if>
											<c:if test="${QX.del == 1 }">
											<li><a style="cursor:pointer;" title="删除" onclick="del('${var.Require_Id}');" class="tooltip-error" data-rel="tooltip" title="" data-placement="left"><span class="red"><i class="icon-trash"></i></span> </a></li>
											</c:if>
										</div>
								</td>
							</tr>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>require/goAdd.do?PId=${pd.PId}';
			 diag.Width = 780;
			 diag.Height = 480;
			 diag.CancelEvent = function(){ //关闭事件
		...
		function del(Id){
			...
			if(confirm("确定要删除?")){ 
				top.jzts();
				var url = "<%=basePath%>require/delete.do?REQUIRE_ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>require/goEdit.do?Require_Id='+Id;
			 diag.Width = 780;
			 diag.Height = 480;
			 diag.CancelEvent = function(){ //关闭事件
		...
		function updatestate(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="更新";
			 diag.URL = "<%=basePath%>require/goUpdateState.do?Require_Id="+Id;
			 diag.Width = 800;
			 diag.Height = 560;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>require/deleteAll.do?tm='+new Date().getTime(),
						    	data: {USER_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>require/excel.do?PId=${pd.PId}';
		}
		</script>
