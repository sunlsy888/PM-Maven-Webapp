...
<body>
	<form action="version/updatepm.do" name="projectForm" id="projectForm" method="post">
		<input name="V_Id" id="V_Id"  type="hidden"  value="${pd.V_Id }"/>
		<input type="hidden" name="PId" id="PId"   value="${pd.PId }"  placeholder="项目编号"/>
		<div id="zhongxin">
		<table> 
			<tr>
		 
				<td><select class="chzn-select" name="VState" id="VState" data-placeholder="请选择状态" style="vertical-align:top;"  >
						<option value="open" <c:if test="${pd.VState=='open'}">selected</c:if> >open</option>  
					 <option value="delay" <c:if test="${pd.VState=='delay'}">selected</c:if> >delay</option>  
					 <option value="closed" <c:if test="${pd.VState=='closed'}">selected</c:if> >closed</option> 
					  	</select>	</td>
