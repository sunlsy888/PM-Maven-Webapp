...
<body>
	<form action="problem/updatepm.do" name="projectForm" id="projectForm" method="post">
		...
		<table> 
			<tr>
				...
				<td><select class="chzn-select" name="State" id="State" data-placeholder="请选择状态" style="vertical-align:top;"  >
				 ...
				 <td>
                   <input class=" date-picker" name="FixPlan" id="FixPlan"  value="${pd.FixPlan}" type="text" data-date-format="yyyy-mm-dd" readonly="readonly"  placeholder="修复计划" title="修复计划"/> 
</td> 
			</tr>
			<tr>
			<td colspan="2"><textarea class="span12" id="Cause" name="Cause"  value="${pd.Cause }"  placeholder="Cause">${pd.Cause }</textarea></td>
			</tr>
			<tr><td colspan="2"><textarea class="span12"  id="Resolvent" name="Resolvent" value="${pd.Resolvent }"  placeholder="Resolvent">${pd.Resolvent }</textarea></td>
			</tr>
			<tr>
						<td id="nr" colspan="2">
							<textarea name="CONTENT" id="CONTENT"  style="display:none"   value="${pd.Remark }" ></textarea>
								<input type="hidden" name="TYPE" id="TYPE" value="0"/>
							 <script id="editor" type="text/plain"    style="width:95%;height:110px;">${pd.Remark }</script>
						</td>
					</tr>    
		<!-- 	<tr>
