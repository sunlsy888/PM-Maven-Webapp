...
function toUserMsg(toU){
 ...
 function creatw() {
			...
			win.on("close",function(){
				websocket.send('LeaveLSYadminqq843855003');//LeaveFHadminqq313596790
				isCreatw = false;
			 });
			...
			function send() {
				...
				if(toUser != ""){content = "lsyadmin886"+toUser+"lsylsyadmin888" + content;}//fhadmin886
				var message = {};
				if (websocket != null) {
					if (input.getValue()) {
						Ext.apply(message, {
...
Ext.define('MessageContainer', {
	...
	tpl : [
			'<div class="l-im-message-warn">​欢迎使用LSY 即时通讯系统。</div>',
			'<tpl for=".">',
